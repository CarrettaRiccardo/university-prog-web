...
<body>
<form method="GET" action="password_recovery.handler" class="form-login text-center ">
    ...
    <c:if test="${not empty param.recovery_error}">
        <div class="alert alert-danger" role="alert">          
            <c:choose>
                ...
                <c:when test="${param.recovery_error == 'aaa'}"> <fmt:message key="login_error_username"/>  </c:when>
    ...
    <c:if test="${not empty param.hasToken}">
        ...
        <input type="password" class="form-control mb-3" id="token" name="password" placeholder="Nuova Password" required autofocus>
