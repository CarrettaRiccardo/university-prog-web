...
<div class="text-center my-4">
    ...
    <button id="loadDate" class="btn btn-gradient btn-block rounded-pill ml-3 mb-3 float-left" <c:if
            test="${empty medico || medico.getId() == 0}"><c:out value="disabled"></c:out></c:if> style="width: 236px">
    ...
    <p class="font-italic text-right position-absolute" style="right: 20px; top: 80px; font-size: 13px; width: 170px">
        (puoi cambiare medico di base nelle impostazioni)</p>
</div>
                        ...
                        <c:choose>
                            <c:when test="${reserv.getIdPaziente() == sessionScope.utente.getId()}">
                                <div class="alert alert-success text-center my-3" style="width: 550px">
                                    <p style="font-style: italic; left: 5px; top: 2px; position: absolute"><c:out
                                            value="${day}"></c:out></p>
                                    <p style="font-weight: bold; right: 5px; top: 2px; position: absolute"><c:out
                                            value="${i}:00"></c:out></p>
                            ...
                            <c:otherwise>
                                <div class="alert alert-danger text-center my-3" style="width: 550px">
                                    <p class="font-italic position-absolute" style="left: 5px; top: 2px"><c:out
                                            value="${day}"></c:out></p>
                                    <p class="font-weight-bold position-absolute" style="right: 5px; top: 2px;"><c:out
                                            value="${i}:00"></c:out></p>
            ...
            <c:if test="${not contains}">
                <div class="alert alert-dark text-center my-3" style="width: 550px">
                    <form action="app/paziente/prenotazioni" method="post">
                        ...
                        <p class="font-italic position-absolute" style="right: 5px; top: 2px;"><c:out
                                value="${i}:00"></c:out></p>
                        ...
                        <c:if test="${date > nowDate}">
                            <button class="btn btn-gradient btn-block rounded-pill text-center ml-3 mt-2"
                                    style="width: 150px">Prenota visita
                            </button>
                        </c:if>
                    </form>
                </div>
