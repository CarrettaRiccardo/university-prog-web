...
<body class="bg">
    ...
    <div id="messageBlock" class="text-center container-fluid"> 
        <div class="alert alert-danger text-center mt-4" role="alert">          
            <c:choose>
                <c:when test="${param.error_code == '400'}"> <fmt:message key="error_code_400"/>  </c:when>
                <c:when test="${param.error_code == '401'}"> <fmt:message key="error_code_401"/>  </c:when>
                <c:when test="${param.error_code == '403'}"> <fmt:message key="error_code_403"/>  </c:when>
                <c:when test="${param.error_code == '404'}"> <fmt:message key="error_code_404"/>  </c:when>
                <c:when test="${param.error_code == '500'}"> <fmt:message key="error_code_500"/>  </c:when>
                <c:otherwise> <fmt:message key="error_code_other"/>  </c:otherwise>
            </c:choose>
        </div>
        <div class="alert alert-dark text-center my-3">          
            <c:choose>
                <c:when test="${param.error_code == '400'}"> <fmt:message key="error_message_400"/>  </c:when>
                <c:when test="${param.error_code == '401'}"> <fmt:message key="error_message_401"/>  </c:when>
                <c:when test="${param.error_code == '403'}"> <fmt:message key="error_message_403"/>  </c:when>
                <c:when test="${param.error_code == '404'}"> <fmt:message key="error_message_404"/>  </c:when>
                <c:when test="${param.error_code == '500'}"> <fmt:message key="error_message_500"/>  </c:when>
                <c:otherwise> <fmt:message key="error_message_other"/>  </c:otherwise>
            </c:choose>
        </div>
